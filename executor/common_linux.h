...
static long syz_ublk_setup_queues(volatile long a0, volatile long a1, volatile long a2, volatile long a3, volatile long a4, volatile long a5, volatile long a6)
{
	...
	for (int q_id = 0; q_id < dev_info->nr_hw_queues; q_id++) {
		...
		for (int j = 0; j < queue_depth; j++) {
			int io_buf_size = dev_info->max_io_buf_bytes;
			char* io_buf_addr = NULL;
			if (posix_memalign((void**)&io_buf_addr, getpagesize(), io_buf_size) != 0) {
				debug("Failed to allocate io_buf for queue: %d dev_id: %d\n", q_id, dev_info->dev_id);
				return -1;
			}
			...
			cmd->q_id = q_id;
			cmd->tag = j;
			cmd->addr = (unsigned long)io_buf_addr;
